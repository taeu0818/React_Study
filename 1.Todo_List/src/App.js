...
const useTasks = (iv /*initialValue*/) => {
    const [tasks, setTasks] = useState(iv);
    useEffect(() => setStorage('tasks', tasks), [tasks]);
    const add = useCallback((task, set) => {
        setTasks(prevTasks => [...prevTasks, {task, complete: false}]);
        set('');
    }, [])
    const toggle = useCallback((id) => setTasks(prevTasks => prevTasks.map((task, i) => i === id ? {...task, complete: !task.complete} : task)), []);
    const remove = useCallback((id) => setTasks(prevTasks => prevTasks.filter((_, i) => id !== i)), []);
    return [tasks, add, toggle, remove];
}

const App = () => {
    const [tasks, addTask, toggleTask, removeTask] = useTasks(getStorage('tasks') ?? []);
    ...
    return (<div className="App h-screen">
        ...
        <div className="relative px-52 pt-28 pb-14">
            ...
            <div className="flex gap-6 mb-5">
                {['All', 'Incomplete', 'Completed'].map(text => <div
            ...
            <div className="grid grid-cols-3 gap-4">
                {tasksMemo.map(({task, complete}, i) => <div
                    ...
                    <div>
                        <div
                            className={`mb-2 ${complete ? 'text-blue-400' : 'text-gray-500 '}`}>{complete ? 'Completed' : 'Incomplete'}</div>
        ...
        <label
            ...
            <div className="px-8 w-full">
                <input type="text"
                       ...
                       onKeyDown={(e) => {
                           if (e.key === 'Enter') addTask(task, setTask)
                       }}/>
            </div>
            <div className="min-w-14 h-full p-1.5">
                <div
                    ...
                    <FaPlus className="text-xl" onClick={() => addTask(task, setTask)}></FaPlus>
                </div>
            </div>
        </label>
    </div>);
